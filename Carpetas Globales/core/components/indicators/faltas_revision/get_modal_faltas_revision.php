<?php
/**
 * Endpoint AJAX para Modal de Faltas Pendientes de Revisión
...
try {
    ...
    $indicator = new \Core\Components\Indicators\List\FaltasRevisionIndicator($conn, '{}');

    // Obtener datos del modal
    $modalData = $indicator->getModalData($userId);

    // Renderizar el template del modal
    include 'faltas_revision_modal.php';

} catch (Exception $e) {
    http_response_code(500);
    error_log("Error en modal de faltas revisión: " . $e->getMessage());
