<?php
/**
 * Endpoint AJAX para Modal de Faltas Pendientes
...
try {
    ...
    $indicator = new \Core\Components\Indicators\List\FaltasPendientesIndicator($conn, '{}');

    // Obtener datos del modal
    $modalData = $indicator->getModalData($userId);

    // Renderizar el template del modal
    include 'faltas_pendientes_modal.php';

} catch (Exception $e) {
    http_response_code(500);
    error_log("Error en modal de faltas: " . $e->getMessage());
