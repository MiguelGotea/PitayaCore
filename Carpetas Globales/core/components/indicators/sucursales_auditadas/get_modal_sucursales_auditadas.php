<?php
/**
 * Endpoint AJAX para Modal de Sucursales Auditadas
...
try {
    ...
    $indicator = new \Core\Components\Indicators\List\SucursalesAuditadasIndicator($conn, '{}');

    // Obtener datos del modal
    $modalData = $indicator->getModalData($userId);

    // Renderizar el template del modal
    include 'sucursales_auditadas_modal.php';

} catch (Exception $e) {
    http_response_code(500);
    error_log("Error en modal de sucursales auditadas: " . $e->getMessage());
