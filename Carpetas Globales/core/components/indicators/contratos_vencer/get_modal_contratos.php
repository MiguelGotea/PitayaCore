<?php
/**
 * Endpoint AJAX para Modal de Contratos por Vencer
...
try {
    ...
    $indicator = new \Core\Components\Indicators\List\ContratosVencerIndicator($conn, '{}');

    // Obtener datos del modal
    $modalData = $indicator->getModalData($userId);

    // Renderizar el template del modal
    include 'contratos_vencer_modal.php';

} catch (Exception $e) {
    http_response_code(500);
    error_log("Error en modal de contratos: " . $e->getMessage());
